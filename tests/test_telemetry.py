...
def test_telemetry_configures_and_reads_raw_access(telemetry_servo) -> None:
    ...
    try:
        ...
        while time.monotonic() < deadline and len(timestamps) < 100:
            with suppress(ILRegisterAccessError):
                ...
                timestamps.extend(
                    int.from_bytes(
                        access[offset : offset + telemetry.descriptor.timestamp_size], "little"
                    )
                    for offset in range(
                        frame_count_size,
                        len(access),
...
def test_telemetry_requires_configuration_before_reading(mocker) -> None:
...
def test_telemetry_rejects_invalid_register_configuration(mocker) -> None:
...
def test_telemetry_rejects_invalid_sample_size(mocker) -> None:
...
def test_telemetry_reader_validates_constructor_arguments(mocker) -> None:
...
def test_telemetry_reader_ignores_transient_register_errors(mocker) -> None:
...
def test_telemetry_reader_forwards_non_empty_accesses(mocker) -> None:
...
def test_telemetry_reader_reports_access_callback_errors(mocker) -> None:
