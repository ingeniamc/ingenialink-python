...
def create_monitoring(connect_to_slave):
    ...
    for idx, key in enumerate(registers_key):
        ...
        servo.monitoring_set_mapped_register(
            idx, reg.address, subnode, reg.dtype.value, MONITORING_CH_DATA_SIZE
        )
    divisor = 1
...
def create_disturbance(connect_to_slave):
    ...
    servo.disturbance_set_mapped_register(
        0, 0x0020, 1, REG_DTYPE.S32.value, DISTURBANCE_CH_DATA_SIZE
    )
...
@pytest.mark.parametrize(
    "ip_address, gateway",
    [("192.168.2.xx", "192.168.2.1"), ("192.168.2.22", "192.168.3.1")],
)
...
def test_status_word_wait_change(connect_to_slave):
    ...
    subnode = 1
    timeout = 0.5
    status_word = servo.read(servo.STATUS_WORD_REGISTERS[subnode], subnode=subnode)
