...
@pytest.mark.parametrize(
    "test2_dtype", [REG_DTYPE.S8, REG_DTYPE.FLOAT, REG_DTYPE.DOMAIN, "Other type"]
)
    ...
    if not isinstance(test2_dtype, REG_DTYPE):
        with pytest.raises(exc.ILValueError):
            EthernetRegister(
                ...
                test2_storage,
                (-20, 20),
                "Monitoring trigger type",
                enums,
                "MONITORING",
                "SUB_CATEGORY_TEST",
                "No description (invent here)",
            )
...
def test_range_ethernet_register(test2_range, test3_dtype):
    ...
    if test2_range == (None, None):
        ...
        test_aux_range = (float(test2_range[0]), float(test2_range[1]))
    else:
        test_aux_range = (int(test2_range[0]), int(test2_range[1]))
