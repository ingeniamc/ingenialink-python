...
class EthernetServo(Servo):
    ...
    SERIAL_NUMBER_REGISTERS = {
        0: EthernetRegister(
            identifier="DRV_ID_SERIAL_NUMBER",
            units="",
            subnode=0,
            address=0x06E6,
            cyclic="CONFIG",
    ...
    SOFTWARE_VERSION_REGISTERS = {
        0: EthernetRegister(
            identifier="DRV_ID_SOFTWARE_VERSION",
            units="",
            subnode=0,
            address=0x06E4,
            cyclic="CONFIG",
    ...
    PRODUCT_ID_REGISTERS = {
        0: EthernetRegister(
            identifier="DRV_ID_PRODUCT_CODE",
            units="",
            subnode=0,
            address=0x06E1,
            cyclic="CONFIG",
    ...
    REVISION_NUMBER_REGISTERS = {
        0: EthernetRegister(
            identifier="DRV_ID_REVISION_NUMBER",
            units="",
            subnode=0,
            address=0x06E2,
            cyclic="CONFIG",
    ...
    def disturbance_write_data(self, channels, dtypes, data_arr):
        data, chunks = self._disturbance_create_data_chunks(
            channels, dtypes, data_arr, ETH_MAX_WRITE_SIZE
        )
        for chunk in chunks:
            self._send_mcb_frame(
                MCB_CMD_WRITE, self.DIST_DATA.address, self.DIST_DATA.subnode, chunk
            )
    ...
    def _monitoring_read_data(self):
        return self._send_mcb_frame(
            MCB_CMD_READ, self.MONITORING_DATA.address, self.MONITORING_DATA.subnode
        )
