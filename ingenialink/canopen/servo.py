...
class CanopenServo(Servo):
    ...
    def _write_raw(self, reg, data, subnode=1):
        ...
        try:
            self._lock.acquire()
            self.__node.sdo.download(_reg.idx, _reg.subidx, data)
        except Exception as e:
            logger.error("Failed writing %s. Exception: %s", str(_reg.identifier), e)
    ...
    def _read_raw(self, reg, subnode=1):
        ...
        try:
            ...
            logger.error("Failed reading %s. Exception: %s", str(_reg.identifier), e)
    ...
    def disturbance_write_data(self, channels, dtypes, data_arr):
        data, chunks = self._disturbance_create_data_chunks(
            channels, dtypes, data_arr, CAN_MAX_WRITE_SIZE
        )
        for chunk in chunks:
