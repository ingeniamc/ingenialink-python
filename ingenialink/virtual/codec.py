...
def deserialize_sdo_frame(frame: bytes) -> dict[str, Any]:
    """Deserialize JSON bytes into an SDO payload dictionary.

    Args:
        frame: The JSON bytes representing the SDO payload.

    Returns:
        The deserialized SDO payload as a dictionary.
    ...
    """
    if len(frame) > VIRTUAL_DRIVE_RECV_BUFFER_SIZE:
        raise ValueError("SDO frame exceeds maximum allowed size")

    payload = json.loads(frame.decode("utf-8"))
    if not isinstance(payload, dict):
        raise ValueError("SDO frame payload must be a dictionary")

    raw_data = payload.get("data")
    if isinstance(raw_data, list):
        if not all(isinstance(item, int) and 0 <= item <= 0xFF for item in raw_data):
            raise ValueError("SDO data array must contain byte values (0..255)")
